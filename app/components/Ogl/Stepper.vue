<template>
    <div class="flex justify-center items-center w-full h-full" v-bind="$attrs">
      <div
        :class="`w-full max-w-2xl p-8 rounded-[2rem] shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] ${stepCircleContainerClassName}`"
      >
      <!-- style="border: 1px solid #505d68" -->
        <div
        ...
        >
          <template v-for="(_, index) in stepsArray" :key="index + 1">
            ...
            >
              ...
              <div v-else-if="getStepStatus(index + 1) === 'active'" class="h-3 w-3 rounded-full bg-zinc-700" />
            ...
            >
              <Motion
                ...
                :animate="
                  currentStep > index + 1
                    ? { width: '100%', backgroundColor: '#33c7e0' }
        ...
        <div v-if="!isCompleted" :class="`w-full ${footerClassName}`">
          <div :class="`flex w-full ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`">
            <button
              ...
              :class="`border-none bg-[#33c7e0] transition-all duration-[350ms] flex items-center justify-center rounded-full text-black font-medium tracking-tight px-3.5 py-1.5 cursor-pointer hover:bg-[#3c6e8e] disabled:opacity-50 disabled:cursor-not-allowed ${currentStep === 1 ? 'opacity-50 cursor-not-allowed' : ''}`"
  ...
  const getStepIndicatorStyle = (step: number) => {
    ...
    switch (status) {
      case 'active':
      case 'complete':
        return { backgroundColor: '#33c7e0', color: '#fff' };
      default:
        return { backgroundColor: 'rgb(240 249 255 / 0.25)', color: '#a3a3a3' };
    }
  };
